...
def single_plot(partial_wave, i):
    ...
    math_labels = [f"${value:.3f}$" for value in perc_steps]
    ...
    if i==5:
        ...
        for n in range(6):
            ...
            plt.plot(energy_lin, ps[:, n] / reference - 1, label=str(math_labels[n]), color=color_arr[n], linewidth=2)

        plt.fill_between(energy_lin, (reference + error_) / reference - 1,  (reference - error_) / reference - 1,
                         color=rgba1, alpha=alp,
                         label=r'N$^3$LO uncertainty')

        for m in range(6):
            #plt.plot(energy_lin, ps[:,m+6]/reference - 1, label=str(perc_steps[m+6]),color=color_arr[m], linewidth=2, linestyle='dashed')
            plt.plot(energy_lin, ps[:, m + 6] / reference - 1, label=math_labels[m + 6], color=color_arr[m],linewidth=2, linestyle='dashed')
...
def multiplot(partial_wave):
    ...
    plt.tight_layout()
    output_file = os.path.join(output_folder, partial_wave + '_percent.pdf')
