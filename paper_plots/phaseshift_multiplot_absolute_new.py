...
mpl.rcParams['axes.labelsize'] = 20  # Adjust the size as desired

# Set the tick label sizes
mpl.rcParams['xtick.labelsize'] = 20  # Adjust the size as desired
mpl.rcParams['ytick.labelsize'] = 20  # Adjust the size as desired
...
def single_plot(x, ax):
    ...
    if x in [0, 2, 4]:
        ax.text(
            0.75, 0.9, partial_wave_label, transform=ax.transAxes,
            fontsize=30, verticalalignment='top', horizontalalignment='left',
            bbox=dict(boxstyle='round', fc='white', ec='black', alpha=0.6)
        )
    elif x in [1, 3]:
        ax.text(
            0.1, 0.2, partial_wave_label, transform=ax.transAxes,
            fontsize=30, verticalalignment='top', horizontalalignment='left',
            bbox=dict(boxstyle='round', fc='white', ec='black', alpha=0.6)
        )
    elif x == 5:
        ax.text(
            0.1, 0.9, partial_wave_label, transform=ax.transAxes,
            fontsize=30, verticalalignment='top', horizontalalignment='left',
            bbox=dict(boxstyle='round', fc='white', ec='black', alpha=0.6)
        )
...
def multiplot():
    ...
    axes[0,1].legend(handles, labels, loc='center', bbox_to_anchor=(0.5, 0.74))
    # You can tweak loc or bbox_to_anchor for a different position

    # Adjust the spacing as needed
    #fig.tight_layout(pad=1, w_pad=0.0, h_pad=1)
    fig.subplots_adjust(left=0.055, right=1, bottom=0.07, top=1, wspace=0.17, hspace=0.08)
    # Save & show
