...
'''
# Set the label sizes for the axes
...
input_path = '/Users/pleazy/PycharmProjects/Phaseshift_Sampler/phaseshifts/weighted_sampling/data_filtered/'
...
#data_folder = '/Users/pleazy/PycharmProjects/magic_quantification/3N_MCMC/resampled_phaseshift_files'
...
data_file_25 = os.path.join(data_folder, data_file_name_25)
mcmc_phaseshifts_all_25 = np.loadtxt(data_file_25)  # shape: (N, len(energy_EM))


data_file_name_1 = f'phaseshifts_{partial_wave}_1000000_to_100_3N_MCMC_1MeV_coupled.dat'
...
def single_plot(ax, i):
    ...
    ax.text(
        0.965, 0.9, plot_labels[i], transform=ax.transAxes, verticalalignment='top', horizontalalignment='right',
    ...
    if i == 6:
        ...
        ax.hist(data_all, bins=bin_edges, alpha=0.6, edgecolor='dimgray', color=rgba1, density=True, label = r'$\mathbf{E}_1$ ppd')
        ax.hist(data_res, bins = bin_edges, alpha=0.5, edgecolor='dimgray', color=rgba2, density=True, label = r'$\mathbf{E}_2$ ppd')
    ...
    if i==6:
        ...
        ax.axvline(percentiles_all[0], color=rgba1, linestyle='dashed', label=r'$68\%$ CI $\mathbf{E}_1$ ppd')
        ax.axvline(percentiles_all[2], color=rgba1, linestyle='dashed')

        ax.axvline(percentiles[0], color=rgba2, linestyle='dashed', label=r'$68\%$ CI $\mathbf{E}_2$ ppd')
...
def legend_plot(i):
    ...
    plt.figlegend(loc='center', ncol=1, labelspacing=1,  bbox_to_anchor=bbox_anchor, fontsize=24)
    plt.axis('off')
...
for partial_wave in partial_waves:
    ...
    for i in range(len(energy_lin)):
        single_plot(i, partial_wave)
    legend_plot(11)
    plt.tight_layout()
    plt.savefig(f'./smarter_plots/phaseshift_distribution_1d_{partial_wave}_20_intial_64_final_vertical.pdf')
plt.show()
'''
...
single_plot(6, partial_wave)
legend_plot(7)
plt.tight_layout()
plt.savefig(f'./plots/phaseshift_distribution_25MeV_compare_legend_presentation.pdf')
'''
...
plt.savefig(f'./plots/phaseshift_distribution_25MeV_compare_{partial_wave}_presentation.pdf')
#plt.savefig(f'./plots/phaseshift_distribution_1d_EM_MCMC_{partial_wave}_3N_resampled_1000000_to_100_100_walkers_19_parameters_fixed_cov.pdf')
#plt.savefig(f'./smarter_plots/phaseshift_distribution_1d_{partial_wave}_20_intial_64_final_vertical.pdf')
plt.show()
'''
...
def multiplot():
    ...
    #fig.tight_layout()
    out_file = os.path.join('./plots', f'phaseshift_distribution_25MeV_compare_{partial_wave}_presentation.pdf')
