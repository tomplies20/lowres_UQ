...
def plot_histograms(pdf_list, labels, colors, shifts=None, n_bins=10, baseline=0, separation=1, y_scale=1):
    ...
    ax.errorbar(x= -411.84, y=2.2, xerr=[[34.56], [34.56]], capsize=10, color='orange', fmt='o', markersize=10, label=r'nonimplausible $68\%$ CI')
    ...
    order = [1, 0, 2]  # custom order indices for two handles
...
pdf1 = np.loadtxt(
    '/Users/pleazy/PycharmProjects/uncertainty_quantification/post_sampling/imsrg_data/energies_calcium_48_25MeV_coupled.txt')[:99, 1]
pdf2 = np.loadtxt(
    '/Users/pleazy/PycharmProjects/uncertainty_quantification/post_sampling/imsrg_data/energies_calcium_48_1MeV_coupled.txt')[
       :, 1]
pdf_list = [pdf1, pdf2]
...
plot_histograms(pdf_list, labels, colors, shifts=shifts, n_bins=10, baseline=0.25, separation=1, y_scale=20)
